...
rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_1', p);  % IMPLEMENT THIS: Decode the received bits
...
for p_ind = 1:length(p_vect)
    ...
    rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_1', p_vect(p_ind));% we send the p_vect(p_ind) as to 
                                                                               % change the probability value for the fucntion;
    BER_case_1_vec(p_ind) = ComputeBER(bit_seq,rec_bit_seq);
end

figure

plot(p_vect,BER_case_1_vec,'x-k','linewidth',2);
xlabel('Values of p')
ylabel('BER')
%%%
...
% System parameters
fs  = 5;    % Number of samples per symbol (bit)[Edieted as the lab said it's supposed to be 5]
...
% Decode bits from received bit sequence
rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_2',p,fs);    % IMPLEMENT THIS: Decode the received bits(we changed fs with p as before                                                             
...
plot(p_vect,BER_case_2_vec,'x-k','linewidth',2);
xlabel('Values of p')
ylabel('BER')
...
fs_vect          = 1:4:100;              % Use this vector to extract different values of fs in your code
...
for fs_ind = 1:length(fs_vect)
    ...
    rec_bit_seq = DecodeBitsFromSamples(rec_sample_seq,'part_2',p,fs_vect(fs_ind));
    BER_case_4_vec(fs_ind) = ComputeBER(bit_seq,rec_bit_seq);
end
%%%
figure

plot(fs_vect,BER_case_4_vec,'x-k','linewidth',2);
xlabel('Values of fs')
ylabel('BER')
title('Part 4')